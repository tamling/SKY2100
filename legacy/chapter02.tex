% =====================================================================
%  Chapter 2  ·  Session 2 --- Data centres, virtualisation and Linux
% =====================================================================
\chapter{Data centres, virtualisation and Linux}
...
\section{The cloud as a distributed system}

The cloud is a \emph{distributed system}: many computers in many buildings,
...
The second is sharing: many customers, called \emph{tenants}, use the same
physical hardware, which is the resource pooling of
Definition~\ref{def:cloud} under the name \emph{multi-tenancy}. The third is
...
\section{Inside the data centre}

The data centre is a building engineered for density, power, cooling and
resilience (Comer, 2021, pp.~37--42). Servers sit in \emph{racks}, a
top-of-rack switch connects the servers of each rack, and racks form
\emph{pods} for which power and cooling are engineered as a unit. Around the
...
the foundation of availability (Definition~\ref{def:cia}). In
Table~\ref{tab:sharedresp} they are provider rows, and accepting those rows
as P means trusting this building.

\section{Traffic and geography}

Traffic in a data centre has two directions (Comer, 2021, p.~44).
\emph{North--south} traffic runs between the data centre and the outside
world, that is, clients calling services. \emph{East--west} traffic runs
...
enough (Comer, 2021, p.~45). The \emph{leaf--spine} design removes the
...
equal-cost path. Where a single cable would not suffice, \emph{link
...
internal, that movement is invisible from outside. This \emph{lateral
...
\begin{tikzpicture}[font=\footnotesize]
  \node[font=\small\itshape, text=kgrey] (net) at (2.25,3.3) {Internet (clients)};
  \draw[kgrey, rounded corners=3pt] (0,0) rectangle (4.5,2.4);
  \node[font=\footnotesize\itshape, text=kgrey, anchor=west] at (0.15,2.15) {data centre};
  \node[box] (s1) at (1.15,0.85) {server};
  \node[box] (s2) at (3.35,0.85) {server};
  \draw[redflow] (net) -- (2.25,2.4);
  \node[font=\footnotesize, text=kristiania, anchor=west] at (2.4,2.85) {north--south};
  \draw[redflow] (s1) -- (s2);
  \node[font=\footnotesize, text=kristiania] at (2.25,1.35) {east--west};
\end{tikzpicture}
\caption{Two directions of traffic. North--south crosses the data-centre
boundary; east--west stays inside it and is where lateral movement takes place
(Session~4).}
\label{fig:nsew}
\end{figure}
...
\begin{definition}[Region, availability zone, region pair]\label{def:regions}
A \textbf{region} describes a geographic area containing one or more data
centres (for example \emph{Norway East}). An \textbf{availability zone}
describes a physically separate data centre, or group of data centres, within
a region, with independent power, cooling and network. A \textbf{region pair}
...
\emph{hyperscalers}, run data centres of this kind at global scale and rent
...
dominance has two consequences. The first is \emph{provider lock-in}: each
provider's services and interfaces differ, so moving between them is costly.
Large organisations therefore sometimes run \emph{multi-cloud}, more than
...
\section{Virtualisation and the hypervisor}
\label{sec:virtualisation}

Virtualisation is what makes this infrastructure rentable.

\begin{definition}[Virtualisation, hypervisor]\label{def:hypervisor}
Virtualisation describes the technique by which one physical machine runs many
isolated \emph{virtual machines} (VMs), each behaving as if it owned the
hardware. The \textbf{hypervisor} describes the software layer that mediates
between them. A \textbf{Type~1} (bare-metal) hypervisor runs directly on the
hardware (ESXi, Xen, Hyper-V) and is what data centres and clouds use. A
\textbf{Type~2} (hosted) hypervisor runs as an application on a host operating
...
gives each guest \emph{virtual hardware}, namely a vCPU that is a share of
...
hypervisor. This is why the server of Example~\ref{ex:vm} appears in two
minutes: creating a VM starts a program rather than assembling hardware.

\section{The Popek--Goldberg criterion}

A hypervisor must meet three requirements at once: \emph{safety}, full
control of the virtualised resources; \emph{fidelity}, so that a program
behaves on the virtual machine exactly as on bare hardware; and
\emph{efficiency}, so that most guest code runs without the hypervisor's
...
\emph{sensitive} instructions, which behave differently in the two modes
(for example I/O or changes to the memory-management unit), and
\emph{privileged} instructions, which trap to the kernel when attempted in
...
instructions that were not privileged: \texttt{POPF} silently failed to
...
\emph{binary translation}: as the guest ran, its kernel code was rewritten
...
guest is modified to call the hypervisor through \emph{hypercalls},
...
trap into a more privileged layer. A \emph{VM escape} breaches that wall:
code breaks out of its guest into the hypervisor, and every guest above it
is exposed. Weak isolation shows first as the \emph{noisy neighbour}, a
...
\begin{example}[Continuation of Example~\ref{ex:vm}]\label{ex:migration}
Because a VM's hardware is virtual, its entire state (memory, disk, devices)
is data; Comer (2021) calls the VM a \emph{digital object} (Comer, 2021,
pp.~63--64). The virtual machine of Example~\ref{ex:vm} can therefore be saved
as a file, a \emph{snapshot}, usable as backup, clone or roll-back point. It can
also move while running: in \emph{live migration} the hypervisor pre-copies the
...
VMs are not the only packaging. A \emph{container} virtualises the operating
...
\section{Linux, the shell and the four pillars}
...
Ubuntu VM; Ubuntu is the \emph{distribution} used here, the kernel plus a
chosen set of tools.
...
kernel/user split of Section~\ref{sec:virtualisation}, give every process
...
Linux has a \emph{monolithic} kernel, one large privileged program that
loads modules; Windows uses a layered \emph{hybrid} NT kernel; macOS runs
...
A server has no graphical desktop, so work runs through a \emph{shell},
usually \texttt{bash}. Four commands suffice for orientation: \texttt{pwd}
(current directory), \texttt{ls -l} (list files with permissions),
\texttt{cd} (change directory) and \texttt{cat} or \texttt{less} (read a
file). The filesystem is one tree, and the Filesystem Hierarchy Standard
fixes where things live: configuration under \texttt{/etc}, log files under
\texttt{/var/log}, user files under \texttt{/home}, programs under
\texttt{/bin} and \texttt{/usr}. Session~7 covers users and permissions.

Linux is multi-user, and one account is special: \texttt{root}, which the
...
commands with \texttt{sudo}, which asks for a password and leaves a trace;
this is least privilege (Session~7) in tool form. Software is installed and
updated through the package manager: \texttt{sudo apt update} refreshes the
catalogue and \texttt{sudo apt upgrade} applies pending updates. On an IaaS
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Resource} & \textbf{Command} & \textbf{Output} \\
\midrule
CPU          & \texttt{lscpu}                 & model, cores, virtualisation flags \\
CPU / load   & \texttt{top} / \texttt{htop} / \texttt{btop} & live processes, CPU~\%, load average \\
Memory       & \texttt{free -h}               & used and free RAM and swap \\
Disk         & \texttt{df -h}                 & free space per filesystem \\
Disk         & \texttt{du -sh} \emph{dir}     & size of a directory \\
Network      & \texttt{ss -tulpn}             & listening sockets and open ports \\
Logs         & \texttt{journalctl}            & system and service log entries \\
\bottomrule
\end{tabular}
\caption{The observation commands used in every laboratory of the semester.}
\label{tab:baseline}
\end{table}

The table has four pillars (CPU, memory, disk, network) plus logs.
\texttt{top} and its successors show live load, and the \emph{load average}
compares demand to the number of cores. \texttt{free -h} shows whether the
system is swapping, a sign of memory pressure. A full disk takes services
down, so \texttt{df} and \texttt{du} matter more than they appear to.
\texttt{ss -tulpn} lists every port the machine listens on, and each of them
is a door an attacker may try, which Session~4 turns into policy.
\texttt{journalctl}, lastly, is the primary log source for Session~10.

\begin{example}\label{ex:spike}
...
shows where the threat--risk--control questions of Definition~\ref{def:trc}
should be asked.
...
\section{Laboratory: recording the baseline}

Exercise~2 has three steps. First, boot the Ubuntu VM prepared after
Session~1, open a terminal and inspect the system with \texttt{lscpu},
\texttt{free -h}, \texttt{df -h}, \texttt{ss -tulpn} and
\texttt{journalctl -xe}. Secondly, install \texttt{htop} and \texttt{btop}
with \texttt{sudo apt install} and compare them with plain \texttt{top}.
...
\paragraph{Reading for this chapter.} Comer, Chapter~4 (Data Center
...
virtualisation in depth: Tanenbaum and Bos, \emph{Modern Operating Systems},
\S7.2 (Requirements for Virtualization, the Popek--Goldberg criterion,
pp.~474--476) and \S7.3 (Type~1 and Type~2 Hypervisors, p.~477); for the
...
deepening: J\o sang, Chapter~3 (System Security, incl.\ virtualisation in \S3.6, pp.~43--62). The Microsoft Learn module
is \emph{Describe Azure architecture and services}; students map regions,
...
(Definition~\ref{def:regions}), and in a management hierarchy. A hypervisor
(Definition~\ref{def:hypervisor}) gives each guest virtual hardware and
...
possible, and so are their risks (Example~\ref{ex:migration}). Linux runs the
cloud, the shell is the interface, and the four pillars of
Table~\ref{tab:baseline}, measured while the system is healthy, are the
...
\begin{exercise}[North--south vs east--west traffic]
...
\begin{exercise}[Which command for which question]
Which command answers each question: Which ports is this machine listening on?
How much memory is free? What has the \texttt{ssh} service logged?
\end{exercise}

\begin{exercise}[Blast radius of one zone]
A deployment runs in one availability zone of one region. Which failures in
Definition~\ref{def:regions} does it survive, and what should be changed
first?
\end{exercise}

\paragraph{Sketch answers.}
{\small
(1)~North--south: data centre~$\leftrightarrow$~outside world; east--west:
...
capacity. (2)~Type~1 runs on bare metal (ESXi, Xen, Hyper-V), beneath every
rented cloud VM; Type~2 runs on a host OS (VMware Workstation, VirtualBox),
beneath the laboratory's Ubuntu VM. (3)~Because tenants are strangers sharing
hardware and only hypervisor mediation separates them. Mild failure: noisy
neighbour; worst case: VM escape, after which every guest is exposed.
(4)~\texttt{ss -tulpn}; \texttt{free -h}; \texttt{journalctl} (for example
\texttt{journalctl -u ssh}). (5)~It survives failures inside other zones
only; a zone-level failure (power, cooling, network of its own zone) takes it
down. First change: spread across availability zones; regional disasters then
motivate the region pair.}

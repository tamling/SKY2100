% =====================================================================
%  Chapter 13  ·  Session 13 --- The integrated case
% =====================================================================
\chapter{The integrated case}
...
running Linux and recorded its \emph{baseline}, the picture of normal that
...
\emph{firewall} under default-deny. Session~5 deployed Nextcloud as a
\emph{container}, choosing the cloud-native, immutable pattern. Session~6
wrapped the transport in \emph{TLS}, and Session~7 hardened \emph{identity}
with MFA and least privilege. Session~8 shielded the application with a
\emph{WAF} and built security into the pipeline with DevSecOps. Session~9
turned the baseline into continuous \emph{monitoring}, and Session~10
protected the \emph{logs} off-host and tamper-evident, so that a compromise
cannot erase its own trace. Session~11 made the data survivable with
\emph{backup} and redundancy, tested by restoring, and Session~12 prepared
the \emph{response} to the incident that gets through, inside a frame of
...
\begin{tikzpicture}[font=\footnotesize]
  \tikzset{layer/.style={draw=kgrey, fill=klight, rounded corners=1.5pt,
    text width=7.4cm, align=left, inner sep=4pt, minimum height=6.2mm}}
  \node[layer] (l1) {Baseline --- know what normal looks like \hfill \textbf{S2}};
  \node[layer, above=1mm of l1] (l2) {Firewall --- default-deny at the perimeter \hfill \textbf{S4}};
  \node[layer, above=1mm of l2] (l3) {Container --- immutable, replaceable workload \hfill \textbf{S5}};
  \node[layer, above=1mm of l3] (l4) {TLS --- confidentiality of data in transit \hfill \textbf{S6}};
  \node[layer, above=1mm of l4] (l5) {Identity --- MFA and least privilege \hfill \textbf{S7}};
  \node[layer, above=1mm of l5] (l6) {WAF \& DevSecOps --- application defence \hfill \textbf{S8}};
  \node[layer, above=1mm of l6] (l7) {Monitoring / IDS --- detect what got through \hfill \textbf{S9}};
  \node[layer, above=1mm of l7] (l8) {Logging --- trustworthy, tamper-evident record \hfill \textbf{S10}};
  \node[layer, above=1mm of l8] (l9) {Backup \& resilience --- survive and recover \hfill \textbf{S11}};
  \node[layer, above=1mm of l9] (l10) {Incident response --- handle the breach \hfill \textbf{S12}};
  \node[redbox, text width=7.4cm, above=1mm of l10] (l11) {Governance, risk \& compliance --- answer for it \hfill \textbf{S12}};
  \node[font=\footnotesize\itshape, text=kgrey, above=1.5mm of l11] {one Nextcloud, hardened control by control};
\end{tikzpicture}
\caption{The hardened service. Each session added one independent layer to
the same Nextcloud; together they form one defensible system.}
\label{fig:stack}
\end{figure}

\section{Mapping to the NIST Cybersecurity Framework}
...
has three levels. At the top are six \emph{Functions}: Govern, Identify,
Protect, Detect, Respond and Recover. Each Function divides into
\emph{Categories}, groups of related outcomes; Protect, for example, has one
...
\emph{Subcategories}, the specific, testable outcomes an organisation
...
Table~\ref{tab:csf} therefore places each Function's Categories beside the
...
\begin{tabular}{@{}p{2.1cm} p{6.2cm} p{5.4cm}@{}}
\toprule
\textbf{Function} & \textbf{Categories (CSF~2.0)} & \textbf{Where this course populates it} \\
\midrule
\textbf{Govern} (GV) &
Organizational Context; Risk Management Strategy; Roles, Responsibilities \&
Authorities; Policy; Oversight; Cybersecurity Supply Chain Risk Management &
Risk register, shared-responsibility model, the NIS2/DORA/ISO~22301 frame,
and supply-chain risk via the SBOM (S12, S8) \\

\textbf{Identify} (ID) &
Asset Management; Risk Assessment; Improvement &
Data classification (S6); asset and dependency inventory / SBOM (S8); risk
assessment and the register (S12) \\

\textbf{Protect} (PR) &
Identity Management, Authentication \& Access Control; Awareness \& Training; Data
Security; Platform Security; Technology Infrastructure Resilience &
IAM, MFA and least privilege (S7); TLS and data protection (S6); the firewall
and segmentation (S4); WAF and secure development (S8); platform hardening:
baseline, containers, patching (S2, S5); backup and resilience (S11) \\

\textbf{Detect} (DE) &
Continuous Monitoring; Adverse Event Analysis &
Monitoring, IDS/IPS and telemetry (S9); trustworthy, tamper-evident logs (S10) \\

\textbf{Respond} (RS) &
Incident Management; Incident Analysis; Incident Response Reporting \&
Communication; Incident Mitigation &
The incident response lifecycle: containment, eradication, analysis and
reporting (S12) \\

\textbf{Recover} (RC) &
Incident Recovery Plan Execution; Incident Recovery Communication &
Tested backup restoration, disaster recovery and business continuity (S11, S12) \\
\bottomrule
\end{tabular}
\caption{The course mapped onto the six CSF~2.0 Functions and their
...
rather than Protect being a single undivided claim.}
\label{tab:csf}
\end{table}
...
\emph{distributed} system, and such systems are complex by nature, however
...
\paragraph{Reading for this chapter.} No new domains. Students revisit the
...
\addcontentsline{toc}{section}{Closing exercise}
...
\emph{Category} of Table~\ref{tab:csf}. If every Function has at least one

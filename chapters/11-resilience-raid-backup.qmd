...
::: {#exr-11-raid-levels}
## RAID levels, interactive
Each box below describes one of the four RAID levels of @def-raid. Drag
each level onto the description that matches it (or click a level, then
click a box), then press **Check**. One of the four is the trap of
@sec-11-redundancy-raid - make sure it lands where its lack of redundancy
belongs.

{{< include _11-raid-levels.qmd >}}
:::

::: {#exr-11-raid-vs-backup}
## RAID or backup?, interactive
@tbl-raidbackup divides the labour between redundancy and backup. Sort each
situation below into the box of the control that handles it: drag it onto a
box, or click the situation and then the box, then press **Check**. Click a
placed item to send it back to the pool.

{{< include _11-raid-vs-backup.qmd >}}
:::

::: {#exr-11-321-numbers}
## The 3–2–1 rule by numbers, interactive
@sec-11-backup states the widely taught rule for keeping backups safe.
Fill the three numbers of the rule into the boxes below and press
**Check** - each line turns green or red on its own.

{{< include _11-321-rule.qmd >}}
:::

::: {#exr-11-resilience-terms}
## Resilience vocabulary, interactive
Five terms from this chapter, five descriptions. Match each term to its
description: click a term and then the description it belongs to (or drag
the term onto it) - a correct pair locks and greys out, a wrong attempt
flashes red. Press **Check** to see how many pairs you have.

{{< include _11-resilience-terms.qmd >}}
:::

::: {#exr-11-provider-redundancy-tf}
## Provider redundancy: true or false?, interactive
@sec-11-provider-redundancy and the Maersk case show how far the
redundancy a provider builds in reaches - and where it stops. Mark each
statement below **True** or **False**, then press **Check**.

{{< include _11-provider-redundancy-tf.qmd >}}
:::

::: {#exr-11-hot-swap}
## Name the practice, interactive
One term from @sec-11-provider-redundancy describes how a data centre's
storage keeps running while a failed disk is replaced. Type it into the
box and press **Check** (small spelling variants are accepted).

{{< include _11-hot-swap.qmd >}}
:::

::: {.callout-important icon=false}
## Related lab: hands-on for this session
Theory done - now build it. This session's practical task is
[Exercise 11: Survive failure - mirror a disk & restore a backup](../labs/exercise-11.qmd)

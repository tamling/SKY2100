...
![Cascading log architecture. Each environment forwards to a central repository; the security-relevant subset is filtered into a retained audit store and fed to the SIEM, the same cascade-and-filter as in Session 9, now applied to storage and cost.](figures/fig-cascade.svg){#fig-cascade fig-alt="Diagram - three boxes (Dev, Test, Prod) each send arrows into a 'central log repository'; an arrow labelled 'filter' leads from it to a 'security / audit store', which in turn feeds a SIEM box."}
...
::: {#exr-10-cascade-order}
## The cascading log architecture, interactive
@fig-cascade shows logs flowing from one layer to the next
(@sec-10-central-collection), and leaving the host at once is what makes
the record trustworthy (@def-logintegrity). Arrange the four steps a log
line takes from the host to the SIEM in order: drag a step to its place, or
use its arrow buttons to move it up and down. Then press **Check**.

{{< include _10-cascade-order.qmd >}}
:::

::: {#exr-10-log-vs-event}
## Log or event, interactive
The Guidance's separation of logs from events decides how each is used
(@sec-10-logs-events). Drag each statement into the box of the record it
describes, or click a statement and then click a box (click a placed
statement to send it back). Then press **Check**.

{{< include _10-log-event.qmd >}}
:::

::: {#exr-10-source-sort}
## Management plane or service log, interactive
The chapter's log sources fall into the two kinds Session 9 named
(@sec-10-log-sources). Drag each item into the box of the kind it belongs
to, or click an item and then click a box (click a placed item to send it
back). Then press **Check**.

{{< include _10-source-buckets.qmd >}}
:::

::: {#exr-10-term-match}
## The logging vocabulary, interactive
From aggregation and the SIEM (@def-logagg) to retention and the chain of
custody (@sec-10-logs-as-evidence), this chapter turns scattered records
into evidence. Click a term, then click the description that fits it (or
start with a description); matched pairs lock and grey out, and a wrong
attempt flashes red. Then press **Check**.

{{< include _10-term-pairs.qmd >}}
:::

::: {#exr-10-true-false}
## True or false: trusting the record, interactive
Ephemeral hosts, append-only stores, Azure's default settings and the CIA
property behind log integrity (@def-logintegrity): judge each statement by
clicking **True** or **False**, then press **Check**; wrong rows are marked
red so you can retry.

{{< include _10-truefalse.qmd >}}
:::

::: {#exr-10-retention-numeric}
## The forensic window, interactive
Forensics after a breach begins with the Azure Activity Log
(@sec-10-logs-events), but only for as long as the platform keeps it. Enter
the default retention period in days, then press **Check** (or Enter).

{{< include _10-retention-numeric.qmd >}}
:::

::: {.callout-important icon=false}
## Related lab: hands-on for this session
Theory done - now build it. This session's practical task is
[Exercise 10: Reconstruct the attack - build a timeline & report](../labs/exercise-10.qmd)
